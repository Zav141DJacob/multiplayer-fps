...
impl Texture {
    pub fn new(bytes: &[u8]) -> anyhow::Result<Self> {
        let textures = image::io::Reader::new(Cursor::new(bytes)).with_guessed_format()?.decode()?.into_rgba8();
        ...
        for j in 0..height {
            for i in 0..width {
                let r = v[((i + j * width) * 4 + 0) as usize] as f32 / 255.;
    ...
    pub fn get(&self, i: i32, j: i32, idx: i32) -> Color {
        let x = self.texture
            [(i + idx * self.size as i32 + j * (self.size * self.count) as i32) as usize];
        x
    }

    pub fn texture_column(
        &self,
        texid: i32,
        texcoord: f32,
        column_height: f32,
