...
impl AnimatedTexture {
    ...
    pub fn register_state(&mut self, name: &str, states: Vec<Vec<usize>>, frame_time: f32) {
        self.states.insert(
            name.to_string(),
            AnimationMap { 
                states,
    ...
    pub fn get_state(&'static self, cur_animation_state: &str) -> AnimatedTextureState {
        AnimatedTextureState { 
            tex: self, 
            cur_state: self.states.get(cur_animation_state).expect("No animation states exist, with that name, or empty"), 
            cur_frame: 0, 
            frame_time_mult: 1.0, 
...
impl AnimatedTextureState {
    ...
    pub fn get_sprite(&mut self, look_angle: f32, cur_time: f32) -> Option<&TextureSampler> {
        self.get_cur_animation_frame(cur_time);

        self.tex.sprite_sheet.get(
            self.get_angled_frame(look_angle)
        )
    }
    ...
    fn get_cur_animation_frame(&mut self, cur_time: f32){
        ...
        if self.current_frame_accumulator >= frame_time {
            ...
            if (self.cur_state.states.len()-1) <= self.cur_frame {
                self.cur_frame = 0;
            } else {
                self.cur_frame += 1;
            }
    ...
    fn get_angled_frame(&mut self, look_angle: f32) -> usize { 
        let cur_frame_vec_size = self.cur_state.states.get(self.cur_frame).expect("No cur_frame").len() as f32;

        let index = ((look_angle / (2.0 * PI)) * cur_frame_vec_size) as i32;

        index.rem_euclid(cur_frame_vec_size as i32) as usize
